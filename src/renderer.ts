...
export function renderField(fieldName: keyof Convict.SchemaObj<unknown> | 'name', name: string, obj: Convict.SchemaObj<unknown>): string {
    switch (fieldName) {
        case 'arg':
            return `${obj.arg ?? '-'}`;
        case 'doc':
...
export function renderDoc<T = any>(schema: Convict.Schema<T>, order: string[] = defaultOrder): string {
